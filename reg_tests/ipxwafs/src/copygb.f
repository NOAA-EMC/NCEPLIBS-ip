...
C  LOOP UNTIL DONE
     ...
     &                  KM,MM,KRMX,KPDSM,KGDSM,KENSM,IRET)
          ...
          IF(IRET.NE.0) THEN
            ...
            IRET=0
          ENDIF
cggg  the zeus compiler found a problem with mm not being defined before being
cggg  used by the 'max' statement below.  i think mm is only used when the
cggg  merge option is on.  this regression test does not use the merge option
cggg  right now.  so simply set mm to zero, which is what it is set to by
cggg  default on ibm.
        ELSE
          MM=0
        ENDIF
...
cggg  this is where mm can be used before it is defined.
          MF=MAX(M1,MB,MM)
...
c    &                  KGDS1,IB1,L1,F1,KGDS1F,IB1F,L1F,F1F,IRET)
          CALL IPXWAFS (1,K1,K1F,1,
     &                  KGDS1,F1,KGDS1F,F1F,IRET)
cggg  ipxwafs does not calculate bitmaps on the full (or reglr) grid.
cggg  this sample data does not have a bitmap. set the bitmap variables as false.
          ib1f=0
          l1f=.false.
        ENDIF
        IF(IRET.EQ.0) THEN
...
c    &                    KGDS2,IB2,L2,F2,KGDS2F,IB2F,L2F,F2F,IRET)
            CALL IPXWAFS (-1,K2,K2F,1,
     &                    KGDS2,F2,KGDS2F,F2F,IRET)
cggg  ipxwafs does not calculate bitmaps on the thinned grid.
cggg  this sample data does not have a bitmap. set the bitmap variables as false.
            ib2=0
            l2=.false.
          ENDIF
