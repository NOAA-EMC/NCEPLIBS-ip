 SUBROUTINE POLATEV3(IPOPT,GDTNUMI,GDTMPLI,GDTLENI, &
...
!           GDTNUMO=GDTNUMO-255, IN WHICH CASE THE NUMBER OF POINTS
...
!
! USAGE:    CALL POLATEV3(IPOPT,GDTNUMI,GDTMPLI,GDTLENI, &
!                     GDTNUMO,GDTMPLO,GDTLENO, &
!                     MI,MO,KM,IBI,LI,UI,VI, &
!                     NO,RLAT,RLON,CROT,SROT,IBO,LO,UO,VO,IRET)
!
...
!     RLAT     - REAL (MO) INPUT LATITUDES IN DEGREES (GDTNUMO<0)
!     RLON     - REAL (MO) INPUT LONGITUDES IN DEGREES (GDTNUMO<0)
!
!   OUTPUT ARGUMENT LIST:
!     NO       - INTEGER NUMBER OF OUTPUT POINTS
!     RLAT     - REAL (MO) OUTPUT LATITUDES IN DEGREES (GDTNUMO>=0)
!     RLON     - REAL (MO) OUTPUT LONGITUDES IN DEGREES (GDTNUMO>=0)
...
!
 INTEGER                         :: GDTNUMO2, IJKGDS1, IJKGDSA(20)
...
!  COMPUTE NUMBER OF OUTPUT POINTS AND THEIR LATITUDES AND LONGITUDES.
 ...
 ELSE
   ...
   ALLOCATE(DUM4(MO))
   ALLOCATE(DUM5(MO))
   CALL GDSWZD(GDTNUMO2,GDTMPLO,GDTLENO,-1,MO,FILL,XPTS,YPTS, &
               RLON,RLAT,NO,1,CROT,SROT,0,DUM1,DUM2,DUM3,DUM4,DUM5)
   DEALLOCATE(DUM1,DUM2,DUM3,DUM4,DUM5)
   IF(NO.EQ.0) IRET=3
 ENDIF
 CALL CHECK_GRIDS3V(GDTNUMI,GDTMPLI,GDTLENI,SAME_GRID)
 IF(.NOT.SAME_GRID) THEN
   ...
   ALLOCATE(DUM4(MI))
   ALLOCATE(DUM5(MI))
   CALL GDSWZD(GDTNUMI,GDTMPLI,GDTLENI, 0,MI,FILL,XPTI,YPTI, &
               RLOI,RLAI,NV,1,CROI,SROI,0,DUM1,DUM2,DUM3,DUM4,DUM5)
   DEALLOCATE(DUM1,DUM2,DUM3,DUM4,DUM5)
 ENDIF
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
!  SET PARAMETERS
...
!  LOCATE INPUT POINTS AND COMPUTE THEIR WEIGHTS AND ROTATIONS
   ...
   IF(WB.NE.0) THEN
     ...
     ALLOCATE(DUM6(NO))
     ALLOCATE(DUM7(NO))
     CALL GDSWZD(GDTNUMO,GDTMPLO,GDTLENO, 1,NO,FILL,XPTB,YPTB, &
                 RLOB,RLAB,NV,0,DUM1,DUM2,0,DUM3,DUM4,DUM5,DUM6,DUM7)
     CALL GDSWZD(GDTNUMI,GDTMPLI,GDTLENI,-1,NO,FILL,XPTB,YPTB, &
                 RLOB,RLAB,NV,0,DUM1,DUM2,0,DUM3,DUM4,DUM5,DUM6,DUM7)
...
!  COMPUTE OUTPUT BITMAPS AND FIELDS
 DO K=1,KM
   ...
   ENDDO
 ENDDO
 IF(GDTNUMO.EQ.0) CALL POLFIXV(NO,MO,KM,RLAT,RLON,IBO,LO,UO,VO)
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 END SUBROUTINE POLATEV3
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 SUBROUTINE CHECK_GRIDS3V(GDTNUM,GDTMPL,GDTLEN,SAME_GRID)
!$$$  SUBPROGRAM DOCUMENTATION BLOCK
!
...
!
! USAGE:  CALL CHECK_GRIDS3V(GDTNUM,GDTMPL,GDTLEN,SAME_GRID)
!
!   INPUT ARGUMENT LIST:
!     GDTNUM  - INTEGER GRID DEFINITION TEMPLATE NUMBER.
!               CORRESPONDS TO THE GFLD%IGDTNUM COMPONENT OF THE
!               NCEP G2 LIBRARY GRIDMOD DATA STRUCTURE.
!     GDTMPL  - INTEGER (GDTLENI) GRID DEFINITION TEMPLATE ARRAY.
...
!$$$
 IMPLICIT NONE
!
 INTEGER,        INTENT(IN   ) :: GDTNUM, GDTLEN
 INTEGER(KIND=4),INTENT(IN   ) :: GDTMPL(GDTLEN)
!
 LOGICAL,        INTENT(  OUT) :: SAME_GRID
!
 INTEGER, SAVE                 :: GDTNUM_SAVE=-9999
 INTEGER, SAVE                 :: GDTLEN_SAVE=-9999
 INTEGER, SAVE                 :: GDTMPL_SAVE(1000)=-9999
!
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 SAME_GRID=.FALSE.
 IF(GDTNUM==GDTNUM_SAVE)THEN
   IF(GDTLEN==GDTLEN_SAVE)THEN
     IF(ALL(GDTMPL==GDTMPL_SAVE(1:GDTLEN)))THEN
       SAME_GRID=.TRUE.
     ENDIF
   ENDIF
 ENDIF
!
 GDTNUM_SAVE=GDTNUM
 GDTLEN_SAVE=GDTLEN
 GDTMPL_SAVE(1:GDTLEN)=GDTMPL
 GDTMPL_SAVE(GDTLEN+1:1000)=-9999
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
