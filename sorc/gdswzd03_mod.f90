   ...
   IF(IOPT.EQ.0.OR.IOPT.EQ.1) THEN
     DO N=1,NPTS
       IF(XPTS(N).GE.XMIN.AND.XPTS(N).LE.XMAX.AND. &
         ...
         IF(LROT)  CALL GDSWZD03_VECT_ROT(IROT,DLON,AN,CROT(N),SROT(N))
         IF(LMAP)  CALL GDSWZD03_MAP_JACOB(H,DR,DLON,RLAT(N),AN,FILL,DXS,DYS, &
                                           XLON(N),XLAT(N),YLON(N),YLAT(N))
         IF(LAREA) CALL GDSWZD03_GRID_AREA(DR,RLAT(N),AN,FILL,DXS,DYS,AREA(N))
       ELSE
         RLON(N)=FILL
         RLAT(N)=FILL
       ENDIF
