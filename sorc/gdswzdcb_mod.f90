   ...
   IF(IOPT.EQ.0.OR.IOPT.EQ.1) THEN
     DO N=1,NPTS
       ...
       IF(XPTF.GE.XMIN.AND.XPTF.LE.XMAX.AND. &
         ...
         ENDIF
         NRET=NRET+1
         IF(LROT) CALL GDSWZDCB_VECT_ROT(IROT,CLAT0,CLAT,CLON,CLATR,SLAT0, &
                                         SLATR,SLAT,RLON0,RLON(N),CROT(N),SROT(N))
         IF(LMAP) CALL GDSWZDCB_MAP_JACOB(FILL,RLON(N),CLAT0,CLAT,CLATR, &
                                          CLON,RLON0,DLATS,DLONS,SLAT0,SLAT, &
                                          XLON(N),XLAT(N),YLON(N),YLAT(N))
         IF(LAREA) CALL GDSWZDCB_GRID_AREA(FILL, CLATR, DLATS, DLONS, AREA(N))
       ELSE
         RLON(N)=FILL
         RLAT(N)=FILL
       ENDIF
