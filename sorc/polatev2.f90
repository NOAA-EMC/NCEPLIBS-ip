 SUBROUTINE POLATEV2(IPOPT,GDTNUMI,GDTMPLI,GDTLENI, &
...
!
! USAGE:    CALL POLATEV2(IPOPT,GDTNUMI,GDTMPLI,GDTLENI, &
!                         GDTNUMO,GDTMPLO,GDTLENO, &
!                         MI,MO,KM,IBI,LI,UI,VI, &
!                         NO,RLAT,RLON,CROT,SROT,IBO,LO,UO,VO,IRET)
!
...
!     RLAT     - REAL (MO) OUTPUT LATITUDES IN DEGREES (IF GDTNUMO>=0)
!     RLON     - REAL (MO) OUTPUT LONGITUDES IN DEGREES (IF GDTNUMO>=0)
!     CROT     - REAL (NO) VECTOR ROTATION COSINES (IF GDTNUMO>=0)
!     SROT     - REAL (NO) VECTOR ROTATION SINES (IF GDTNUMO>=0)
!                (UGRID=CROT*UEARTH-SROT*VEARTH;
!                 VGRID=SROT*UEARTH+CROT*VEARTH)
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 CALL CHECK_GRIDS2V(GDTNUMI,GDTMPLI,GDTLENI,GDTNUMO,GDTMPLO,GDTLENO, &
...
!  COMPUTE NUMBER OF OUTPUT POINTS AND THEIR LATITUDES AND LONGITUDES.
   IF(GDTNUMO.GE.0) THEN
     ...
     CALL GDSWZD(GDTNUMO,GDTMPLO,GDTLENO, 0,MO,FILL,XPTS,YPTS,RLON,RLAT, &
                 NO,1,CROT,SROT,0,DUM3,DUM4,DUM5,DUM6,DUM7)
     DEALLOCATE(DUM3,DUM4,DUM5,DUM6,DUM7)
     IF(NO.EQ.0) IRET=3
   ENDIF
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
!  LOCATE INPUT POINTS
   ...
   ALLOCATE(DUM6(MI))
   ALLOCATE(DUM7(MI))
   CALL GDSWZD(GDTNUMI,GDTMPLI,GDTLENI, 0,MI,FILL,XPTI,YPTI,RLOI,RLAI, & 
               NV,1,CROI,SROI,0,DUM3,DUM4,DUM5,DUM6,DUM7)
   DEALLOCATE(DUM3,DUM4,DUM5,DUM6,DUM7)
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 SUBROUTINE CHECK_GRIDS2V(GDTNUMI,GDTMPLI,GDTLENI,GDTNUMO,GDTMPLO,GDTLENO, &
                          SAME_GRIDI, SAME_GRIDO)
!$$$  SUBPROGRAM DOCUMENTATION BLOCK
!
...
!
! USAGE:  CALL CHECK_GRIDS2V(GDTNUMI,GDTMPLI,GDTLENI,GDTNUMO,GDTMPLO, &
