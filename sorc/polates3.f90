...
!  DO SUBSECTION OF GRID IF KGDSO(1) IS SUBTRACTED FROM 255.
 ...
 ELSE
   ...
   ALLOCATE(DUM1(MO))
   ALLOCATE(DUM2(MO))
   CALL GDSWIZ(KGDSO,-1,MO,FILL,XPTS,YPTS,RLON,RLAT,NO,0,DUM1,DUM2)
   DEALLOCATE(DUM1,DUM2)
   IF(NO.EQ.0) IRET=3
 ENDIF
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
!  LOCATE INPUT POINTS AND COMPUTE THEIR WEIGHTS
   ...
   IF(WB.NE.0) THEN
     ...
     ALLOCATE(DUM1(NO))
     ALLOCATE(DUM2(NO))
     CALL GDSWIZ(KGDSO, 1,NO,FILL,XPTB,YPTB,RLOB,RLAB,NV,0,DUM1,DUM2)
     CALL GDSWIZ(KGDSI,-1,NO,FILL,XPTB,YPTB,RLOB,RLAB,NV,0,DUM1,DUM2)
     DEALLOCATE(DUM1,DUM2)
 ...
 KM_LOOP : DO K=1,KM
   ...
   N_LOOP : DO N=1,NO
     ...
     ELSEIF (MSPIRAL.GT.1) THEN
       ...
       LAT(1)=RLAT(N)
       LON(1)=RLON(N)
       CALL GDSWIZ(KGDSI,-1,1,FILL,XXX,YYY,LON,LAT,NV,0,DUM1,DUM2)
       DEALLOCATE(DUM1,DUM2)
       XX=XXX(1)
       YY=YYY(1)
       IF(NV.EQ.1)THEN
