...
!!
!! @param IDIR     - INTEGER TRANSFORM OPTION
!!                (+1 TO EXPAND THINNED FIELDS TO FULL FIELDS)
!!                (-1 TO CONTRACT FULL FIELDS TO THINNED FIELDS)
!! @param M1       - INTEGER SKIP NUMBER BETWEEN THINNED GRID FIELDS
!! @param M2       - INTEGER SKIP NUMBER BETWEEN FULL GRID FIELDS
!! @param KM       - INTEGER NUMBER OF FIELDS TO TRANSFORM
!! @param[out] KGDS1    - INTEGER (200) GDS PARMS OF THINNED GRID IF IDIR>0
...
!! @param[out] IB1      - INTEGER (KM) THINNED BITMAP FLAGS IF IDIR>0
!! @param[out] L1       - LOGICAL*1 (M1,KM) THINNED BITMAP FIELDS IF IDIR>0
!! @param[out] F1       - REAL (M1,KM) THINNED GRID FIELDS IF IDIR>0
!! @param[out] KGDS2    - INTEGER (200) GDS PARMS OF FULL GRID IF IDIR<0
!! @param[out] IB2      - INTEGER (KM) FULL BITMAP FLAGS IF IDIR<0
!! @param[out] L2       - LOGICAL*1 (M1,KM) FULL BITMAP FIELDS IF IDIR<0
!! @param[out] F2       - REAL (M2,KM) FULL GRID FIELDS IF IDIR<0
